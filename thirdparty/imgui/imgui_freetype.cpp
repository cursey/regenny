...
//  2019/02/09: added RasterizerFlags::Monochrome flag to disable font anti-aliasing (combine with ::MonoHinting for
//  best results!) 2019/01/15: added support for imgui allocators + added FreeType only override function
//  SetAllocatorFunctions(). 2019/01/10: re-factored to match big update in STB builder. fixed texture height waste.
//  fixed redundant glyphs when merging. support for glyph padding. 2018/06/08: added support for
...
void FreeTypeFont::BlitGlyph(
    const FT_Bitmap* ft_bitmap, uint32_t* dst, uint32_t dst_pitch, unsigned char* multiply_table) {
...
bool ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags) {
    ...
    // We allocate in chunks of 256 KB to not waste too much extra memory ahead. Hopefully users of FreeType won't find
...
void ImGuiFreeType::SetAllocatorFunctions(
    void* (*alloc_func)(size_t sz, void* user_data), void (*free_func)(void* ptr, void* user_data), void* user_data) {
